...
class Registrar_Adapter_TPPWholesale extends Registrar_AdapterAbstract
{
    ...
    // Verbose diagnostics — off by default, toggled in admin config.
    ...
    public function __construct(array $options)
    {
        ...
        // Debug logging — off by default, toggled in admin config.
     ...
     *
     * Always announces the outgoing request and its raw response via
     * debugLog() (only written when Debug Logging is enabled). Any
     * transport-level failure (timeout, connection reset, DNS issue) is
     * always logged, regardless of the debug setting, since that's a real
     * failure worth knowing about.
     *
     * In test mode, logs the URL and returns a fake OK response without
    ...
    private function httpGet(string $url): string
    {
        ...
        try {
            ...
            // getContent(false) — do not throw on a non-2xx status, we want
            // to see and log the body regardless of the HTTP status code.
            $content    = trim($response->getContent(false));
 
            $this->debugLog("RECEIVED (HTTP {$statusCode}, " . strlen($content) . ' bytes): ' . $content);
 
            return $content;
        } catch (\Throwable $e) {
            $this->log('TPP httpGet TRANSPORT EXCEPTION: ' . get_class($e) . ': ' . $e->getMessage());
            throw new Registrar_Exception('TPP Wholesale HTTP request failed: ' . $e->getMessage());
        }
    }
    ...
    private function createContact(string $sessionId, Registrar_Domain $domain): string
    {
        ...
        // Log the contact fields being used — distinct from the raw HTTP
        // dump, this is a friendlier structured summary and always-on
        // since it's small and genuinely useful for support/debugging.
    ...
    public function renewDomain(Registrar_Domain $domain): bool
    {
        ...
        // Extend the stored expiration time by the renewal period, same reasoning
        // as registerDomain() — don't rely solely on a later sync to pick this up.
    ...
    public function getDomainDetails(Registrar_Domain $domain): Registrar_Domain
    {
        ...
        if (isset($data['ExpiryDate'])) {
            ...
            // TPP didn't return an expiry date for some reason — fall back to a
            // computed value so the record is never left with a missing expiry.
            $years = $domain->getRegistrationPeriod();
            $domain->setExpirationTime(strtotime("+$years year"));
        }
 
        // TPP's Details response does not reliably include a registration/created
        // date field. If we don't already have one stored (e.g. from
        // registerDomain() having run previously), fall back to "now" so the
        // record is never left blank — matches the test-mode behaviour above.
        if (!$domain->getRegistrationTime()) {
            $domain->setRegistrationTime(time());
        }
